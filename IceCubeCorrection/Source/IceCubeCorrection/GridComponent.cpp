...
void UGridComponent::ShowGrid()
{
    ...
    {
        DRAW_LINE(settings.startLocation + FVector(100, 0, 0) * i,
            settings.startLocation + FVector(100, 0, 0) * i + FVector(0, 100, 0) * (settings.horizontalLine - 1),
            FColor::Red, 4);
    }
    for (int i = 0; i < settings.horizontalLine; i++)
    {
        DRAW_LINE(settings.startLocation + FVector(0, 100, 0) * i,
            settings.startLocation + FVector(0, 100, 0) * i + FVector(100, 0, 0) * (settings.verticalLine - 1),
            FColor::Red, 4);
    }
}
...
{
    if (GetActorLocation().X == settings.startLocation.X + 100 * (settings.verticalLine - 1))
        return;
...
{
    if (GetActorLocation().Y == settings.startLocation.Y + 100 * (settings.horizontalLine - 1))
        return;
